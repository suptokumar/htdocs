...
{{--                     <li class="nav-item" data-item="others">
        ...
        <div class="main-content-wrap sidenav-open d-flex flex-column">
            ...
            <div class="breadcrumb">
                ...
                <ul>
                    <li><a href="{{ url('/clouds') }}">Json Editor</a></li>
                    <li>Overview</li>
                </ul>
            </div>
...
@endphp
            				...
            				@endphp
                                    <div class="card-header header-elements-inline">
                                        <h6 class="card-title ul-collapse__icon--size ul-collapse__right-icon mb-0">
                                            <a data-toggle="collapse" class="text-default" href="#accordion-item-icon-right-{{$i}}" aria-expanded="false">{{$file}}</a>
                                        </h6>
                                    ...
                                    <div id="accordion-item-icon-right-{{$i}}" class="collapse {{$i==0?'show':''}}" data-parent="#accordionRightIcon" style="">
                                        <div class="card-body">
                                            <textarea name="" class="form-control" id="saveip{{$i}}" cols="30" rows="10">{{$int[explode(".",$file)[0]]}}</textarea>
                                            <button class="btn btn-secondary mt-2" onclick="save({{$i}},'{{$file}}', this)">Save</button>
                                        </div>
                                    </div>
                                    @php
                                    	$i++;
            		...
            		@endforeach
<script>
	function save($id, $file, $this)
	{
		$($this).html("Saving");
		$.ajax({
			url: '{{ url('/filesaver') }}',
			type: 'POST',
			data: {file: $file, _token: "{{csrf_token()}}", text: $("#saveip"+$id).val()},
		})
		.done(function(data) {
			$($this).html(data);
			setTimeout(function(){
