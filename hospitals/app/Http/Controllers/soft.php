...
$t1+= intval($value->test_total);
    ...
    public function invice(){
        $a = service::where("type",'=','X-RAY')->get();
        $b = service::where("type",'=','ULTRA')->get();
        $c = service::where("type",'=','ECG')->get();
        $d = service::where("type",'=','Pathology')->get();
        return view("invoice",["a"=>$a,"b"=>$b,"c"=>$c,"d"=>$d]);
    }
    ...
    public function memo4($user){
        if($users = admission::find($user)){
            ...
            // }
            // }
            return view("ememo4",["user"=>$users,"tests"=>$testsa]);
        }else{
            return back();
        }
    }
    	...
    	if (Auth::check()) {
            ...
            if ($request->get("log")=='Admin') {
                ...
                foreach ($admin as $key => $ad) {
                    if ($request->get("pass".$ad->id)==$ad->pass) {
                    }else{
    ...
    public function accounted(Request $request){
        if (User::updateOrCreate(["id"=>$request->get("id")],["name"=>$this->null_pointer($request->get("name")),"email"=>$this->null_pointer($request->get("email")),"phone"=>$this->null_pointer($request->get("phone")),"address"=>$this->null_pointer($request->get("address")),"role"=>$this->null_pointer($request->get("role")),"password"=>Hash::make($request->get("password")),"pass"=>$this->null_pointer($request->get("password"))])) {
...
WHEN contact LIKE '___________$search%' THEN 12 END, name ASC;
    ...
    function appoinmentlist(Request $request){
        ...
        $and = "apoinments WHERE (name LIKE '%$search%' OR token LIKE '%$search%') $dt $con";
...
public function tick(Request $request){
    ...
    function admissionlist(Request $request){
        ...
        $and = "admissions WHERE (name LIKE '%$search%' OR id LIKE '%$search%') $dt $con";   
        // $page = 1;
        $page = $request->get("page");
        $limit = 15;
        $from = ($page-1)*$limit;
    ...
    function reportsd(Request $request){
        ...
        foreach ($resultt as $key => $value) {
            ...
            if($em_user = emargency::find($resultt[$key]->em_id)){
        ...
        foreach ($results as $key => $value) {
            ...
            if($em_user = admission::find($results[$key]->em_id)){
            $results[$key]->names = $em_user->name;
            $results[$key]->contact = $em_user->contact;
            }else{
            $results[$key]->names = '';
            $results[$key]->contact = '';
            }
...
$amount = 0;
            ...
            $c =  " created_at > '$date1' AND created_at < '$date2'";
        $and = "expenses WHERE $c ";
       $rspb = DB::select("SELECT * FROM ".$and);
        foreach ($rspb as $key => $value) {
            $amount+=$rspb[$key]->amount;
        }
    ...
    return json_encode([$result,[count($total)+count($totals), $page, $limit],$amount]);
    }
    ...
    function adload_userdetails(Request $request){
        $user= $request->get("user");
    ...
    function emargencylist(Request $request){
        $search = $request->get("search");
        $and = "emargencies WHERE (name LIKE '%$search%' OR id LIKE '%$search%' OR contact LIKE '%$search%' OR id LIKE '%$search%' OR reffered LIKE '%$search%' OR consultant LIKE '%$search%')";
...
if ($date1!='') {
    ...
    function accountlist(Request $request){
        ...
        $total = DB::select("SELECT id FROM ".$and." ORDER BY name ASC;
            ");
    return json_encode([$result,[count($total), $page, $limit],Auth::id()]);
    }
