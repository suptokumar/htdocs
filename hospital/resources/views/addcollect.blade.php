  ...
  <div class="form-group row">
    <div class="col-sm-12">
      ...
      <button type="submit" class="btn btn-success float-right">Collect</button>
        <?php endif ?>
        <?php if ($user->discharge==1): ?>
          <h2>User Discharged.</h2>
        <?php endif ?>
    </div>
  </div>
</form>
...
<form action="{{ url('/admission/discharge') }}" method="POST">
  ...
  <div class="form-group row">
    <div class="col-sm-12">
        <input type="hidden" name="id" value="{{$user->id}}">
      <a class="btn btn-danger float-right" onclick="sdonweo()">Discharge</a>
    </div>
  </div>
</form>
<script>
