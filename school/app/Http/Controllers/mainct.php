    ...
    public function upcoming($id){
        if ($user = User::find($id)) {
            ...
            foreach ($class as $key => $value) {
                if ($value->repeat=='') {
                    array_push($data,[date("Y-m-d h:i a", strtotime($value->starting)),$value->subject,$value->duration]);
                }else{
                    $crt = time();
                    $mxt = time()+3600*24*30;
                    ...
                    array_push($data,[date("Y-m-d h:i a", $nxt),$value->subject,$value->duration]);
                    $nxt+=24*3600;
                        }else{
                            $nxt+=24*3600;
                        }
...
<td>'.$value[2].' minutes</td>
    ...
    public function reqs_t(Request $request){
        ...
        $req = new requestd;
        $req->notification=Auth::user()->email.' Request to change a schedule. <br> Timezone: '.$request->get("timezone").'<br>Time: '.$request->get("time").' '.$request->get("date").'<br>Repeat:'.$request->get("repeat");
        $req->sql=$sql;
        $req->users=1;
    ...
    public function approvereq(Request $request){
        ...
        if($req->users==2){
            ...
            $clt = $cls;
            if ($cls->delete()) {
                $this->send($clt->t_id," The request delete class ". $clt->ras." was Approved.");
                $this->send($clt->s_id," The request delete class ". $clt->ras." was Approved.");
                $this->send("Admin"," The request delete class ". $clt->ras." was Approved.");
                $req->delete();
                return "Approved";
            }else{
                return "Failed to Approve";
    ...
    public function get_requests(Request $request){
        ...
        foreach ($result as $key => $value) {
            ...
            array_push($starting_time, [date("Y-m-d h:ia", $pre_v)]);
        ...
        $next_class = date("l",$lst_c);
            ...
            date_default_timezone_set(Auth::user()->timezone);
               ...
               if ($lst_c-time() < (3600*24*1)) {
                ...
                $upcoming_class .= "<span style='border: 1px solid blue; padding: 3px; display: block;'>".date("D, Y-m-d h:ia", $lst_c)."</span>";
