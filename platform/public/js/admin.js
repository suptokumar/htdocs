...
function load_users(page, order){
	...
	.done(function(data) {
      ...
      for (var i = 0; i < d[0].length; i++) {
        ...
        if (row['type']==2) {
            if(row['hours']/60<1){
                body+= "<section style='background: #ff7b7b; color: black;'>"+(row['hours']/60)+"</section>";
            }else{
                body+= (row['hours']/60);
            }
...
function load_notifications(page){
  var urls = url("platform/notifications");
  var token = $("#token").val();
  var search = $("#snod410").val();
  $.ajax({
    url: urls,
