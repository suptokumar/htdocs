...
class mainct extends Controller
{
    ...
    public function upcoming($id){
        if ($user = User::find($id)) {
            $class = course::where("s_id","=",$user->id)->get();
            $data = [];
            date_default_timezone_set($user->timezone);
            foreach ($class as $key => $value) {
                if ($value->repeat=='') {
                    array_push($data,[date("D, M d,Y h:i a", strtotime($value->starting)),$value->subject,$value->duration]);
                }else{
                    $crt = time();
                    $mxt = time()+3600*24*30;
                    ...
                    array_push($data,[date("D, M d,Y h:i a", $nxt),$value->subject,$value->duration]);
                    $nxt+=24*3600;
                        }else{
                            $nxt+=24*3600;
                        }
...
<td>'.$value[2].' minutes</td>
    ...
    public function reqs_t(Request $request){
        $sql = $request->get("date")." ".$request->get("time")."```".$request->get("timezone")."```".$request->get("repeat")."```".$request->get("del");
        $req = new requestd;
        $req->notification=Auth::user()->email.' Requested to change a schedule. <br> Timezone: '.$request->get("timezone").'<br>Time: '.$request->get("time").' '.$request->get("date").'<br>Repeat:'.$request->get("repeat");
        $req->sql=$sql;
        $req->users=1;
        $req->requested=Auth::user()->email;
        $req->approved='';
        if ($req->save()) {
            $this->send("Admin","You have a new request from ".Auth::user()->email);
            $this->send(Auth::user()->id,"You made a new request.");
            $fon = course::find($request->get("del"));
            
            $req = new requestd;
        $req->notification='It is a rollback Request to change a schedule. You should approve it when a class is done. <br> Timezone: '.$fon->timezone.'<br>Time: '.$fon->lastclass.'<br>Repeat:'.$request->get("repeat");
        $req->sql=$sql;
        $req->users=1;
        $req->requested=Auth::user()->email;
        $req->approved='';
        $req->save();
            ...
            if (Auth::user()->type==2) {
            $this->send($fon->t_id,Auth::user()->email." sent a request to change the schedule of class '".$fon->title."'.<br> Timezone: ".$request->get('timezone')."<br>Time: ".$request->get('time')."<br>Repeat:".$request->get('repeat'));
            }else{
            $this->send($fon->s_id,Auth::user()->email." sent a request to change the schedule of class '".$fon->title."'.<br> Timezone: ".$request->get('timezone')."<br>Time: ".$request->get('time')."<br>Repeat:".$request->get('repeat'));

        }
            return back()->with("success","Your Request sent!");
        }else{
            return back()->with("message","Failed to sent request.");
        }
    }
    ...
    public function approvereq(Request $request){
        ...
        if($req->users==2){
            ...
            if($cls->repeat==''){
                ...
                $repeat = explode(",",$cls->repeat);
                $next_class=strtotime(empty($cls->lastclass)?$cls->starting:$cls->lastclass);
                while(true){
                    if(in_array(date('l',$next_class),$repeat)){
                        $cls->lastclass=date("Y-m-d H:i:s",$next_class);
                        $cls->save();
                        return "Approved";
                    }else{
                        $next_class+=3600*24;
            ...
            if ($cls->delete()) {
                $this->send($clt->t_id," The request of delete class ". $clt->ras." was Approved.");
                $this->send($clt->s_id," The request of delete class ". $clt->ras." was Approved.");
                $this->send("Admin"," The request of delete class ". $clt->ras." was Approved.");
                $req->delete();
                return "Approved";
            }else{
                return "Failed to Approve";
            ...
            $cls->timezone = $main[1];
            date_default_timezone_set($main[1]);
            $cls->starting = date("Y-m-d H:i:s",strtotime($main[0].":00"));
            $cls->lastclass = date("Y-m-d H:i:s",strtotime($main[0].":00")-3600*24);
            $cls->repeat=$main[2];
            if ($cls->save()) {
                $this->send($cls->t_id," The request to change class schedule for ". $cls->ras." was Approved.");
                $this->send($cls->s_id," The request to change class schedule for ". $cls->ras." was Approved.");
                $this->send("Admin"," The request to change class schedule for ". $cls->ras." was Approved.");
                $req->delete();
                return "Approved";
            }else{
                return "Failed to Approve";
    ...
    public function reqs_d(Request $request){
        $sql = $request->get("note")."```".$request->get("del");
        $req = new requestd;
            $fon = course::find($request->get("del"));
        $req->notification=Auth::user()->email.' Request to cancel a class ('.$fon->ras.'). <br> '.$request->get("note");
        $req->sql=$sql;
        $req->users=2;
        $req->requested=Auth::user()->email;
        $req->approved='';
        if ($req->save()) {
            ...
            $this->send(Auth::user()->id,"You made a new request.");
            if (Auth::user()->type==2) {
            $this->send($fon->t_id,Auth::user()->email." sent a request to cancel the class(".$fon->ras.") '".$fon->title);
            }else{
            $this->send($fon->s_id,Auth::user()->email." sent a request to cancel the class '".$fon->title);
    ...
    public function request_cancel($id){
        if ($course = course::find($id)) {
            ...
            return view("request_cancel",["id"=>$id]);
        }else{
            return back();
        }
    }
    public function request_change($id){
        if ($course = course::find($id)) {
            if (Auth::user()->type!=3) {
                if ($course->s_id!=Auth::id() AND Auth::user()->type==2) {
                    return back();
            ...
            return view("request_change",["id"=>$id, "course"=>$course]);
        }else{
            return back();
        }
    }
...
if ($request->hasFile('national_id_front')) {
        $r =  $request->file('national_id_front')->getPathName();
// Save the image

$path = public_path()."/image/";
$national_id_front = time().rand().$request->file('national_id_front')->getClientOriginalName();
...
if ($request->hasFile('national_id_back')) {
        $r =  $request->file('national_id_back')->getPathName();
// Save the image

$path = public_path()."/image/";
$national_id_back = time().rand().$request->file('national_id_back')->getClientOriginalName();
...
if ($request->hasFile('image')) {
        $r =  $request->file('image')->getPathName();
// Save the image
...
$user->image = $image;
    ...
    public function create(Request $request){
        if ($find = User::where("phone","=",$this->null_pointer($request->get("email")))->first()) {
            return back()->with("message","The phone number exists!");
        }
        if ($find = User::where("email","=",$this->null_pointer($request->get("email")))->first()) {
            return back()->with("message","The email exists!");
        }
        if (!in_array($request->get("timezone"),timezone_identifiers_list())) {
            return back()->with("message","Timezone not valid!");
        }
        $type = $request->get("type");
        $name = $this->null_pointer($request->get("name"));
...
// functions for the admin to manage class
    ...
    function get_classes(Request $request){
        ...
        $utc = [];
        foreach ($result as $key => $value) {
            $result[$key]->repeat = $value->repeat!=''?substr($value->repeat, 1):"";
...
public function n(Request $request){
    if (Auth::user()->type==3) {
        ...
        return json_encode(array(count($result),count($results)));
