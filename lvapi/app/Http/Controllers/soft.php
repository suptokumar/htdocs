...
WHEN phone LIKE '__________$search%' THEN 11
 END,
    ...
    {
        if ($this->power(Auth::id(),"deposit",0)) {
    ...
    public function deletedeposit(Request $request)
    {
        if ($this->power(Auth::id(),"deposit",0)) {
            ...
            if ($deposit->delete()) {
                $this->send($deposit->username,$deposit->phone, "Your Deposit request of ".$deposit->amount." ".$deposit->currency." was deleted", Auth::user()->name,0);
                return __("Deleted");
            }else{
                return __("Failed");
            }
    ...
    public function approvedeposit(Request $request)
    {
        if ($this->power(Auth::id(),"deposit",0)) {
            ...
            if ($deposit->save()) {
                $this->send($deposit->username,$deposit->phone, "Your Deposit request of ".$deposit->amount." ".$deposit->currency." was approved", Auth::user()->name,0);
                return __("Approved");
            }else{
                return __("Failed");
            }
    ...
    {
        if ($this->power(Auth::id(),"deposit",0)) {
            $user= User::find($this->getuserbyphone($request->id)->id);
    ...
    public function adddeposit(Request $request)
    {
        if ($this->power(Auth::id(),"deposit",0)) {
         ...
         if ($request->hasFile('screenshot'))
            ...
            $id->exchange = $request->get('exchange');
            $id->username = $this->getuserbyphone($request->get('username'))->name;
            $id->phone = $request->get('phone');
    ...
    public function load_deposit(Request $request)
    {
        ...
        $type = $request->get("type");
        if ($type=='') {
        $and = "deposits WHERE (`username` LIKE '%$search%' OR `phone` LIKE '%$search%')";
        }else{
        $and = "deposits WHERE (`username` LIKE '%$search%' OR `phone` LIKE '%$search%') AND status='$type'";
        }
