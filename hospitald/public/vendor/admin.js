...
function reports(page){
  ...
  .done(function(data) {
    ...
    var due = 0;
      var d = JSON.parse(data);
      var body = '<table class="table" id="adfea"> <thead class="thead-light"> <tr> <th scope="col">ID</th> <th scope="col">From</th> <th scope="col">Tests</th> <th scope="col">Total</th><th scope="col">Paid</th><th scope="col">Due</th><th scope="col">Date</th><th scope="col">User</th></thead> <tbody>';
      for (var i = 0; i < d[0].length; i++) {
        var row = d[0][i];
        if (due15=="due") {
...
function admission(page){
  ...
  .done(function(data) {
      ...
      for (var i = 0; i < d[0].length; i++) {
        ...
        body+= "<a class='btn btn-success' style='margin: 1px' href='"+url("admission/discharge/"+row["id"])+"'>Discharge</a>";
        }
