...
@csrf
  ...
  <div class="form-group row">
    ...
    <div class="col-sm-9">
      <input type="text" name="name" required="" autofocus="" autocomplete="off" class="form-control" value="{{isset($doctor)?$doctor->name:''}}" id="name" placeholder=" Name">
    </div>
  </div>
  <div class="form-group row">
    <label for="age" class="col-sm-3 col-form-label">Age</label>
    <div class="col-sm-9">
      <input type="text" name="age" required="" class="form-control" autocomplete="off" value="{{isset($doctor)?$doctor->age:''}}" id="age" placeholder="Age">
    </div>
  </div>
  <div class="form-group row">
  ...
  <div class="form-group row">
    ...
    <div class="col-sm-9">
     <select name="reffered" class="form-control" id="reffered">
      	<option value="" {{isset($doctor)?($doctor->reffered==''?'selected':''):''}}>--Select--</option>
       @foreach ($doc as $d)
        <option value="{{$d->name}}" {{isset($doctor)?($doctor->reffered==$d->name?'selected':''):''}}>{{$d->name}}</option>
        @endforeach
      </select>
    </div>
  </div>
  ...
  <div class="form-group row">
    ...
    <div class="col-sm-9">
      <input type="date" name="date" class="form-control" value="{{isset($doctor)?$doctor->date:date("Y-m-d")}}" id="date" placeholder="date">
    </div>
  </div>
  <div class="form-group row">
    <label for="consultant" class="col-sm-3 col-form-label">Consultant</label>
    <div class="col-sm-9">
      <select name="consultant" class="form-control" required="" id="consultant">
	...
	<div class="row" style="text-align: center;">
		<div class="col-sm-10">
      <a href="javascript:void(0)" class="btn btn-primary float-right" onclick="load_token(document.getElementById('date').value,document.getElementById('consultant').value)">Load Token</a>
		</div>
    </div><br>
  <div class="form-group row">
    <label for="token" class="col-sm-3 col-form-label">Token</label>
    <div class="col-sm-9">
      <input type="text" name="token" class="form-control" autocomplete="off" value="{{isset($doctor)?$doctor->token:''}}" id="token" placeholder="Token">
    </div>
  </div>
  <div class="form-group row">
    <label for="total" class="col-sm-3 col-form-label">Total Amount</label>
    <div class="col-sm-9">
...
<input type="date" id="dt10" autocomplete="off" required="" value="{{date("Y-m-d")}}" onchange="appoinment(1)" style="padding: 10px; border: 1px solid #ccc; outline: none;" placeholder="Search">
<div class="dk">
